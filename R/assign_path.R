...
#' # (needs to match PR2 v5 9 rank's taxonomy structure):
#' path_func_exa2 <- unite(taxo_exa, "taxonomy", all_of(colnames(taxo_exa)[2:10]), sep=";") %>%
#'   assign_path()
...
#' path_func_exa3 <- assign_path(taxo_exa, ref=DBc_main)
#' dplyr::count(path_func_exa3, assigned_at_rank)
#' dplyr::count(path_func_exa3, main_functional_class)
#' }
#' @import dplyr
#' @importFrom magrittr %>%
